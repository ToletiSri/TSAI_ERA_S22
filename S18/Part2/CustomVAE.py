...
class VAE(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        # Pick random target label for image about 25% of times i.e train 25% times with incorrect label for image
        random_number = random.randint(1, 100)
        if random_number <= 25:
            combined_features = torch.cat((x_encoded, label_ohe_random), dim=1) #get OHE for label features
        else:
            combined_features = torch.cat((x_encoded, label_ohe), dim=1) #get OHE for label features
